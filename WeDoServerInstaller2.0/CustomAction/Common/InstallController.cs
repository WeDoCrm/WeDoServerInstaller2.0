...
namespace CustomAction
{
    public class InstallController
    {
        ...
        public bool CheckWinpcapInstalled()
        {
            ...
            try
            {
                isAppInstalled = Utils.IsAppInstalled(MACHINE_NAME, WINPCAP_NAME);
            }
            catch (Exception ex)
            {
        ...
        public bool InstallWinpcap()
        {
            ...
            try
            {
                ...
                Stream exeStream = _assembly.GetManifestResourceStream(ConstDef.WINPCAP_RES_FILE);

                FileInfo fileInfoOutputFile = new FileInfo(Application.StartupPath + ConstDef.WINPCAP);
                ...
                OnWriteLog(string.Format("{0} 파일을 복사합니다.", ConstDef.WINPCAP_INSTALLNAME));
                
                P = Process.Start(Application.StartupPath + ConstDef.WINPCAP);
                P.WaitForExit();
                OnWriteLog("Winpcap 설치 완료");
            }
            catch (Exception ex)
            {
                OnWriteLog("WinPcap파일 생성중 에러발생");
                Logger.error("WinPcap파일 생성중 에러발생 : " + ex.ToString());
                return false;
            }
        ...
        /// <summary>
        /// DB Port값을 config파일에만 적용.
        /// </summary>
        /// <returns></returns>
        public bool UpdateDbPort()
        {
            Logger.info("UpdateDbPort");
            bool result = false;
        ...
        public bool UpdateCompanyCode()
        {
            ...
            try
            {
                ...
                if (companyCdChanged || !configFileHandler.PrevConfigExists())
                {
                    ...
                    configFileHandler.SetValue("COM_NAME", companyName);
                }
                else
                {
